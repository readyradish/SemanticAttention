...
def plot_heatmap_multi(all_scores, default_score, save_dir, title, gold_indexs, attention_parts, attention_level):
    ...
    if len(all_scores) == 4:
        fig, axes = plt.subplots(1, 4, figsize=(11, 4), squeeze=False)
...
if __name__ == "__main__":
    ...
    settings = ["position_level_no_docid", "position_level_have_docid", 
                "token_level_have_docid", "token_level_have_docid_with_reversed_idx"
                "token_level_have_docid_with_ascending_posword"]
    subtitles = ["relative attention with docid", "relative attention with docid", 
                 "absolute attention with docid", "absolute attention with reversed docid"
                 "absolute attention with posword"]
